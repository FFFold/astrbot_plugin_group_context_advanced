...
- **插件归档**: AstrBot 框架已在 [PR #8226](https://github.com/AstrBotDevs/AstrBot/pull/8226) 中原生实现了本插件的全部功能，此插件已结束历史使命，不再维护。
...
### 修复
- 提高群聊消息缓存处理器优先级，避免被前置插件阻塞导致上下文记录缺失

### 优化
- 将 `[GCPLUGIN]` system 提示词移动到本轮注入的群聊历史之后，减少下一轮清理标记时对大段历史前缀缓存命中的影响
- 群聊缓存项记录 `message_id`，仅在明确匹配当前触发消息时排除该消息，避免当前消息在插件上下文和 AstrBot 当前 prompt 中重复出现
- 匹配到当前触发消息时只消费到该消息为止，保留其后新进入的群消息给下一轮请求使用

## v0.2.1

### 修复
- 群聊上下文中的消息时间改为优先使用平台消息原始时间戳，而不是插件实际处理时间，避免前置插件耗时较长时上下文时间偏晚
- 兼容秒级和毫秒级时间戳；时间戳异常时回退当前时间，避免消息记录失败
